...
function saveOptions() {
  ...
  chrome.storage.sync.set({
    ...
    pageTitleInFile: document.getElementById('pageTitleInFile').checked,
...
function restoreOptions() {
  chrome.storage.sync.get({
    ...
    document.getElementById('pageTitleInFile').checked = items.pageTitleInFile;
  ...
  chrome.runtime.sendNativeMessage(HOST_APPLICATION_NAME, testConnectivityPayload, function(response) {
    if (chrome.runtime.lastError) {
      ...
      document.getElementById('directoryMessage').innerHTML =
        ...
        '</div>' +
        '<div>' +
          'Follow setup instructions outlined <a href="https://github.com/bobbyrne01/save-text-to-file-chrome#installation" target="_blank">here</a>.' +
        '</div>';
